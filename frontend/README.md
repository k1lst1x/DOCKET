...
and `DEMO` in the Amplify console; `amplify.yml` passes them to the server runtime.
...
`POST /api/groups/[slug]/join`, `GET /api/stats`, `POST /api/auth/request`,
`GET /api/auth/verify`. Group and role always come from the URL or the session,
...
- Members live in memory (`src/lib/store.ts`): they reset on restart and are not
