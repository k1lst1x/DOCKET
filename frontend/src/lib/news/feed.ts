...
const SOURCES: SourceSpec[] = [
  ...DISTRICTS.map<SourceSpec>((district) => ({
    key: `google-${slugOf(district)}`,
    name: `Google News: ${district}`,
    url: googleNews(`(${DISTRICT_ALIASES[district].slice(0, 3).map((a) => `"${a}"`).join(" OR ")}) Fremont when:30d`),
    ttlMs: 10 * MINUTE,
    kind: "article",
    keep: "local",
    district,
  ...
  { key: "google-fremont", name: "Google News: Fremont", url: googleNews('"Fremont, CA" OR "Fremont, Calif" when:3d'), ttlMs: 10 * MINUTE, kind: "article", keep: "fremont" },
