...
async function replace(file, before, after) {
  ...
  // Normalize Windows checkouts (CRLF) so the adapters match on every OS.
        ...
        <div className="page py-10 sm:py-16">
          <div className="mx-auto max-w-md rounded-2xl border border-rule bg-white p-6 sm:p-10">
            ...
            <p className="mt-2 text-base text-ink-soft">
              Sign-in isn&apos;t available on this preview. You can browse every group without an account.
            </p>
          </div>
        </div>
      </main>
...
for (const route of ['groups', 'places', 'chat', 'find', 'start', 'g/niles-neighbors', 'g/niles-neighbors/join', 'news']) {
